...
function findAgentFiles(dir: string): string[] {
  ...
  const agentIndicators = [
    ...
    'Chain', 'AgentExecutor', 'LCEL', 'RunnableSequence', 'RunnableParallel',
 ...
 */
function findLocalCalls(content: string, sourceFile: string): Edge[] {
  const edges: Edge[] = [];
...
function checkHandoffValidation(node: AgentNode, type: string): Issue[] {
  ...
  switch (type) {
    case 'call':
      if (!node.handlers.includes('validate') && 
    ...
    case 'context-pass':
      if (node.tools.some(t => t.toLowerCase().includes('secret') || 
...
function checkToolAuthorization(node: AgentNode, adjList: Record<string, string[]>): Issue[] {
  const issues: Issue[] = [];
...
function detectPrivilegeEscalation(node: AgentNode, adjList: Record<string, string[]>): Issue[] {
  ...
  const hasAdminTools = node.tools.some(t => 
    ...
    if (!visited[nodeName]) {
      if (hasCycle(nodeName)) {
        issues.push({
          ...
          severity: 'high',
          file: '',
          message: 'Detected circular dependency between agents - risk of infinite loops or resource exhaustion',
...
function validateToolChains(graph: { nodes: AgentNode[], edges: Edge[] }): Issue[] {
  ...
  for (const edge of graph.edges) {
    ...
    const hasSensitiveTools = sourceNode.tools.some(t => 
