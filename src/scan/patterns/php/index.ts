...
export default definePatterns([
	...
	},
	{
		...
		regex: /extract\s*\(\s*\$_(GET|POST|REQUEST|COOKIE)/g,
		severity: "high",
	...
	{
		name: "SQL string concatenation (PHP)",
		regex:
			/(?:mysql|mysqli)_query\s*\([^)]*\.|mysql_query\s*\([^)]*\$|mysqli_query\s*\([^)]*\$|PDO.*->query\s*\([^)]*\.|->query\s*\([^)]*\$/g,
		severity: "critical",
