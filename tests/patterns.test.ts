...
describe('Pattern Scanner', () => {
  describe('Pattern Imports', () => {
    ...
    it('should have total pattern count matching sum of all language patterns', () => {
      const stats = patternStats.byLanguage;
      const sum = stats.rust + stats.javascript + stats.python + stats.go + stats.ruby + stats.php + stats.common + stats.config;
      assert.strictEqual(patternStats.total, sum, 'Total should match sum of all language patterns');
    });
    ...
    it('should have patterns organized by severity', () => {
      ...
      assert.strictEqual(patternStats.total, severitySum, 'Total should match sum of severity counts');
    });
  });
  ...
  describe('Pattern Validation', () => {
    it('should have all patterns with required fields', () => {
      for (const pattern of DANGEROUS_PATTERNS) {
        ...
        assert.ok(['critical', 'high', 'medium', 'low'].includes(pattern.severity), 'Pattern should have valid severity');
  ...
  describe('JavaScript Patterns', () => {
    const jsPatterns = DANGEROUS_PATTERNS.filter(p => 
      p.fileTypes && p.fileTypes.includes('.js')
    );
    ...
    it('should detect SQL injection via template literals', () => {
      const sqlPattern = DANGEROUS_PATTERNS.find(p => 
        p.name === 'SQL template literal injection'
      );
    ...
    it('should detect SSRF via fetch with user input', () => {
      const ssrfPattern = DANGEROUS_PATTERNS.find(p => 
        p.name === 'SSRF via fetch'
      );
  ...
  describe('Python Patterns', () => {
    const pyPatterns = DANGEROUS_PATTERNS.filter(p => 
      p.fileTypes && p.fileTypes.includes('.py')
    );

    it('should detect SQL injection via f-strings', () => {
      const sqlPattern = DANGEROUS_PATTERNS.find(p => 
        p.name === 'SQL f-string injection'
      );
    ...
    it('should detect SSTI via render_template_string', () => {
      const sstiPattern = DANGEROUS_PATTERNS.find(p => 
        p.name === 'SSTI via render_template_string'
      );
  ...
  describe('Go Patterns', () => {
    const goPatterns = DANGEROUS_PATTERNS.filter(p => 
      p.fileTypes && p.fileTypes.includes('.go')
    );

    it('should detect SQL injection via format strings', () => {
      const sqlPattern = DANGEROUS_PATTERNS.find(p => 
        p.name && p.name.includes('SQL') && p.fileTypes?.includes('.go')
      );
      if (sqlPattern) {
  ...
  describe('Rust Patterns', () => {
    const rustPatterns = DANGEROUS_PATTERNS.filter(p => 
      p.fileTypes && p.fileTypes.includes('.rs')
    );
  ...
  describe('File Type Filtering', () => {
    it('should only match patterns on appropriate file types', () => {
      const jsEvalPattern = DANGEROUS_PATTERNS.find(p => 
        p.name === 'eval() usage' && p.fileTypes?.includes('.js')
      );
  ...
  describe('Spot Check Problematic Patterns', () => {
    it('should detect command injection in JavaScript', () => {
      const spawnPattern = DANGEROUS_PATTERNS.find(p => 
        p.name === 'child_process spawn with shell'
      );
    ...
    it('should detect path traversal vulnerabilities', () => {
      const pathPattern = DANGEROUS_PATTERNS.find(p => 
        p.name && p.name.includes('Path traversal')
      );
      if (pathPattern) {
    ...
    it('should detect hardcoded secrets patterns', () => {
      const secretPatterns = DANGEROUS_PATTERNS.filter(p => 
        p.name && p.name.toLowerCase().includes('secret')
      );
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle case-insensitive regex patterns', () => {
      const casePatterns = DANGEROUS_PATTERNS.filter(p => 
        p.regex.flags.includes('i')
      );
      assert.ok(casePatterns.length > 0, 'Should have case-insensitive patterns');
    });

    it('should handle multi-line regex patterns', () => {
      const multilinePatterns = DANGEROUS_PATTERNS.filter(p => 
        p.regex.source.includes('\\n') || p.regex.flags.includes('s')
      );
